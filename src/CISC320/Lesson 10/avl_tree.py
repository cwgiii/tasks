...
1. Line #27: height was set to 3 when it should be 1, as height represents the 
number of lines below the root
2. Line #72: If statment if root is none should be if root is not none, the 
previous statement would only iterate if there was no root
3. Line #124: Added a the left rotation as well as the right, as well as the return statement 
4. Line #161: Changed granchild from right to left of the root, as that is the proper
algorithm 
5. Line #100: Changed from adding to the right to adding to the left per comment
6. Line #175: Changed from a min function to a max, the max of the difference of heights will
now alert the program if the balance is over -1 or 1
7. Line #144/145: Changed from 2+height to 1+height according to the correct algorithm
"""
